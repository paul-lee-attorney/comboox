...
import "./ILOOKeeper_3.sol";

contract LOOKeeper_4 is ILOOKeeper_3, RoyaltyCharge {
    ...
    function regInvestor(
        ...
        IGeneralKeeper_5(address(_gk)).getLOO().regInvestor(caller, groupRep, idHash);
    }
    ...
    function approveInvestor(
        ...
        IListOfOrders_2 _loo = IGeneralKeeper_5(address(_gk)).getLOO();
    ...
    function revokeInvestor(
        ...
        IGeneralKeeper_5(address(_gk)).getLOO().revokeInvestor(userNo, caller);
    }
    ...
    function _placeSellOrder(
        ...
        (OrdersRepo_2.Deal[] memory deals, GoldChain_2.Order[] memory expired, OrdersRepo_2.Deal memory offer) = 
            IGeneralKeeper_5(address(_gk)).getLOO().placeSellOrder(
                caller,
                classOfShare, 
                seqOfShare, 
                votingWeight, 
        ...
        if (offer.price == 0 && offer.paid > 0) {
            GoldChain_2.Order memory balance;
    ...
    function _closeDeals(OrdersRepo_2.Deal[] memory deals) private {
        ...
        while (len > 0) {
            ...
            OrdersRepo_2.Deal memory deal = deals[len - 1];
            len--;
    ...
    function _restoreOrder(GoldChain_2.Order memory order) private {
    ...
    function _restoreExpiredOrders(GoldChain_2.Order[] memory orders) private {
    ...
    function withdrawInitialOffer(
        ...
        GoldChain_2.Order memory order = 
        ...
        _ros.increaseEquityOfClass(false, classOfShare, 0, 0, order.node.paid);
    }
    ...
    function placeBuyOrder(
        ...
        InvestorsRepo.Investor memory investor = 
            IGeneralKeeper_5(address(_gk)).getLOO().getInvestor(caller);
    ...
    function _placeBuyOrder(
        ...
        (OrdersRepo_2.Deal[] memory deals, GoldChain_2.Order[] memory expired, OrdersRepo_2.Deal memory bid) = 
            IGeneralKeeper_5(address(_gk)).getLOO().placeBuyOrder(
                classOfShare,
                buyer,
                groupRep,
                paid,
