...
async function main() {
	...
	let libBallotsBox = await contractGetter("BallotsBox");
	let libCheckpoints = await contractGetter("Checkpoints");
	let libDelegateMap = await contractGetter("DelegateMap");
	let libDTClaims = await contractGetter("DTClaims");
	let libEnumerableSet = await contractGetter("EnumerableSet");
	let libFRClaims = await contractGetter("FRClaims");

	let libLockersRepo = await contractGetter("LockersRepo");
	let libMembersRepo = await contractGetter("MembersRepo");
	let libMotionsRepo = await contractGetter("MotionsRepo");
	let libOptionsRepo = await contractGetter("OptionsRepo");

	let libPledgesRepo = await contractGetter("PledgesRepo");
	let libRolesRepo = await contractGetter("RolesRepo");
	let libRulesParser = await contractGetter("RulesParser");
	let libSharesRepo = await contractGetter("SharesRepo");
	let libSwapsRepo = await contractGetter("SwapsRepo");
	let libTopChain = await contractGetter("TopChain");

	// ==== Templates ====

	let ia = await contractGetter("InvestmentAgreement");

	let sha = await contractGetter("ShareholdersAgreement");
	let ad = await contractGetter("AntiDilution");
	let lu = await contractGetter("LockUp");
	let da = await contractGetter("DragAlong");
	let ta = await contractGetter("TagAlong");
	let op = await contractGetter("Options");
