...
async function main() {
    ...
    const ad = await readTool("AntiDilution", AD);
    ...
    const da = await readTool("Alongs", DA);
    ...
    const ta = await readTool("Alongs", TA);
    ...
    expect(governingSHA).to.equal(await sha.getAddress());
