...
function parseOption(arr) {
    return {
        head: {
            seqOfOpt: arr[0][0],
            typeOfOpt: typeOfOpt[arr[0][1]],
            classOfShare: arr[0][2],
            rate: ethers.utils.formatUnits(arr[0][3], 4),
            issueDate: parseTimestamp(arr[0][4]),
            triggerDate: parseTimestamp(arr[0][5]),
            execDays: arr[0][6],
            closingDays: arr[0][7],
            obligor: arr[0][8],
        },
        cond: {
            seqOfCond: arr[1][0],
            logicOpr: logicOpr[arr[1][1]],
            compOpr1: compOpr[arr[1][2]],
            para1: ethers.utils.formatUnits(arr[1][3], 4),
            compOpr2: compOpr[arr[1][4]],
            para2: ethers.utils.formatUnits(arr[1][5], 4),
            compOpr3: compOpr[arr[1][6]],
            para3: arr[1][7],
        },
        body: {
            closingDeadline: parseTimestamp(arr[2][0]),
            rightholder: arr[2][1],
            paid: ethers.utils.formatUnits(arr[2][2], 4),
            par: ethers.utils.formatUnits(arr[2][3], 4),
