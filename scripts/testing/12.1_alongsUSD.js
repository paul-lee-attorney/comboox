...
const { getGK, getROA, getGMM, getROS, getROM, getRC, getUSDC, getCashier, getUsdROAKeeper, getUsdKeeper, } = require("./boox");
...
async function main() {
    ...
    const payOffDeal = async (seqOfDeal, numOfSigner) => {
      ...
      tx = await usdKeeper.connect(signers[5]).payOffApprovedDeal(auth, ia.address, seqOfDeal, signers[numOfSigner].address);
