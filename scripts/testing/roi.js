...
const parseDeal = (sn) => {
  ...
  return {
    ...
    paid: longDataParser(formatUnits(BigInt(sn.substring(32, 48)).toString(), 4)),
    price: longDataParser(formatUnits(parseInt(sn.substring(48, 56), 16), 4)),
...
const parseNode = (sn) => {
  ...
  return {
    ...
    paid: formatUnits(BigInt(sn.substring(26, 42)).toString(), 4),
    price: formatUnits(BigInt(sn.substring(42, 50)).toString(), 4),
...
const parseData = (sn) => {
  ...
  return {
    ...
    margin: formatUnits(BigInt(sn.substring(30, 62)).toString(), 18),
...
const parseOrder = (arr) => {
  return {
    ...
    data: {
      ...
      distrWeight: arr[1][4],
      margin: longDataParser(formatUnits(arr[1][5].toString(), 18)),
      state: arr[1][6]
    },
  };
}
