...
contract GMMKeeper_2 is IGMMKeeper, AccessControl {
    using RulesParser for bytes32;

    event DistributeProfits(uint indexed sum, uint indexed seqOfMotion, uint indexed caller);
    ...
    function distributeProfits(
        ...
        while (len > 0) {
            ...
            uint amtOfMember = pointsOfMember * amt / totalPoints;
            
            _gk.saveToCoffer(member, amtOfMember);
            sum += amtOfMember;
