...
library SigsRepo {
    ...
    error SIGR_WrongInput(bytes32 reason);

    error SIGR_WrongState(bytes32 reason);

    error SIGR_Overflow(bytes32 reason);

    error SIGR_WrongParty(bytes32 reason);
    ...
    function addBlank(
        ...
        if (seq == 0) {
            revert SIGR_WrongInput(bytes32("SIGR_ZeroSeqOfDeal"));
        }
        if (acct == 0) {
            revert SIGR_WrongInput(bytes32("SIGR_ZeroAcct"));
        }

        
        if (beBuyer) {
            if (p.sellers.contains(acct)) {
                revert SIGR_WrongInput(bytes32("SIGR_SellerIntendsToBuy"));
            }
            p.buyers.add(acct);
        } else {
            if (p.buyers.contains(acct)) {
                revert SIGR_WrongInput(bytes32("SIGR_BuyerIntendsToSell"));
            }
            p.sellers.add(acct);
        }
    ...
    function signDoc(Page storage p, uint256 acct, bytes32 sigHash) 
        public 
    {
        if (block.timestamp >= getSigDeadline(p) &&
            getSigningDays(p) != 0) {
            revert SIGR_WrongInput(bytes32("SIGR_MissedSigDeadline"));
        }

        if (established(p)) {
            revert SIGR_WrongState(bytes32("SIGR_DocAlreadyEstablished"));
        }
