...
library DTClaims {
    ...
    modifier dealExist(Claims storage cls, uint seqOfDeal) {
        if (!hasClaim(cls, seqOfDeal)) 
            revert DTClaims_WrongState(bytes32("DTC_DealNotExist"));
        _;
    }
    ...
    error DTClaims_WrongInput(bytes32 reason);

    error DTClaims_WrongState(bytes32 reason);
    ...
    function execAlongRight(
        ...
        if (head.seqOfDeal == 0) 
            revert DTClaims_WrongInput(bytes32("DTC_ZeroSeqOfDeal"));
        ...
        if (newClaim.seqOfShare == 0) 
            revert DTClaims_WrongInput(bytes32("DTC_ZeroSeqOfShare"));
    ...
    function getClaimForShare(
        ...
        if (!hasShare(cls, seqOfDeal, seqOfShare)) 
            revert DTClaims_WrongState(bytes32("DTC_ShareNotExist"));
