...
library LockersRepo {
    ...
    error LR_WrongInput(bytes32 reason);

    error LR_WrongState(bytes32 reason);

    error LR_WrongParty(bytes32 reason);
    ...
    function pickupPoints(
        ...
        if (hashLock != keccak256(key)) {
            revert LR_WrongInput(bytes32("LR_WrongKey"));
        }

        Locker storage locker = repo.lockers[hashLock];

        if (block.timestamp >= locker.head.expireDate) {
            revert LR_WrongState(bytes32("LR_LockerExpired"));
        }

        bool flag = true;

        if (locker.body.counterLocker != address(0)) {
            if (locker.head.to != caller) {
                revert LR_WrongParty(bytes32("LR_NotIntendedReceiver"));
            }
    ...
    function withdrawDeposit(
        ...
        if (block.timestamp < locker.head.expireDate) {
            revert LR_WrongState(bytes32("LR_LockerNotExpired"));
        }

        if (locker.head.from != caller) {
            revert LR_WrongParty(bytes32("LR_NotSettlor"));
        }

        if (repo.snList.remove(hashLock)) {
            head = locker.head;
            delete repo.lockers[hashLock];
        } else revert LR_WrongState(bytes32("LR_LockerNotExist"));
    }
