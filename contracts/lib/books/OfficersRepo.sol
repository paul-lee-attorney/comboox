...
library OfficersRepo {
    ...
    error OFR_WrongInput(bytes32 reason);

    error OFR_WrongState(bytes32 reason);

    error OFR_WrongParty(bytes32 reason);
    ...
    modifier isVacant(Repo storage repo, uint256 seqOfPos) {
        if (isOccupied(repo, seqOfPos)) {
            revert OFR_WrongState(bytes32("OFR_PositionOccupied"));
        }
        _;
    }
    ...
    function addPosition(
        ...
        if (pos.title <= uint8(TitleOfOfficers.Shareholder)) {
            revert OFR_WrongInput(bytes32("OFR_ZeroPosition"));
        }
        if (pos.seqOfPos == 0) {
            revert OFR_WrongInput(bytes32("OFR_ZeroSeqOfPos"));
        }
        if (pos.titleOfNominator == 0) {
            revert OFR_WrongInput(bytes32("OFR_ZeroTitleOfNominator"));
        }
        if (pos.endDate <= pos.startDate) {
            revert OFR_WrongInput(bytes32("OFR_EndDateNotLaterStart"));
        }
        if (pos.endDate <= block.timestamp) {
            revert OFR_WrongInput(bytes32("OFR_EndDateNotFuture"));
        }
        ...
        } else if (p.seqOfPos != pos.seqOfPos || p.title != pos.title)
            revert OFR_WrongInput(bytes32("OFR_RemovePosFirst"));
    ...
    function takePosition (
        ...
        if (acct == 0) {
            revert OFR_WrongInput(bytes32("OFR_ZeroAcct"));
        }
        ...
        else revert OFR_WrongInput(bytes32("OFR_PosNotExist"));
    ...
    ) public returns (bool flag)
    {
        ...
        if (acct != pos.acct) {
            revert OFR_WrongParty(bytes32("OFR_NotTheOfficer"));
        }
    ...
    ) public returns (bool flag)
    {
        ...
        uint acct = pos.acct;
        if (acct == 0) {
            revert OFR_WrongInput(bytes32("OFR_EmptyPos"));
        }
