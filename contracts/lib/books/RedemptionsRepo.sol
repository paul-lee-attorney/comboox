...
library RedemptionsRepo {
    ...
    error PDPR_WrongState(bytes32 message);

    error PDPR_WrongInput(bytes32 message);

    error PDPR_WrongParty(bytes32 message);
    ...
    modifier redeemableClass(
        ...
        if (!isRedeemable(repo, class)) 
            revert PDPR_WrongState(bytes32("PDPR_NotRedeemableClass"));
        _;
    }
    ...
    modifier redeemablePack(
        ...
        if (!repo.classes[class].packsList.contains(seqOfPack)) 
            revert PDPR_WrongState(bytes32("PDPR_NotRedeemablePack"));
        _;
    }
    ...
    function redeem(
        ...
        if (info.value == 0) 
            revert PDPR_WrongInput(bytes32("PDPR_ZeroValue"));
    ...
    function getRequest(
        ...
        if (!repo.classes[class].packs[seqOfPack].sharesList.contains(seqOfShare)) {
            revert PDPR_WrongState(bytes32("PDPR_WrongSeqOfShare"));
        }
