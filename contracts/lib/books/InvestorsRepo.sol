...
library InvestorsRepo {
    ...
    error IR_WrongInput(bytes32 reason);

    error IR_WrongState(bytes32 reason);

    error IR_WrongParty(bytes32 reason);
    ...
    modifier investorExist(
        ...
        if (!isInvestor(repo, acct)) {
            revert IR_WrongParty(bytes32("IR_InvestorNotExist"));
        }
        _;
    }
    ...
    function regInvestor(
        ...
        if (idHash == bytes32(0)) {
            revert IR_WrongInput(bytes32("IR_ZeroIdHash"));
        }
        
        uint40 user = uint40(userNo);

        if (user == 0) {
            revert IR_WrongInput(bytes32("IR_ZeroUserNo"));
        }
    ...
    function approveInvestor(
        ...
        if (investor.state == uint8(StateOfInvestor.Approved)) {
            revert IR_WrongState(bytes32("IR_AlreadyApproved"));
        }
    ...
    function revokeInvestor(
        ...
        if (investor.state != uint8(StateOfInvestor.Approved)) {
            revert IR_WrongState(bytes32("IR_NotApproved"));
        }
