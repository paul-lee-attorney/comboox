...
library FilesRepo {
    ...
    error FR_WrongInput(bytes32 reason);

    error FR_WrongState(bytes32 reason);

    error FR_Overflow(bytes32 reason);
    ...
    modifier onlyRegistered(Repo storage repo, address body) {
        if (!repo.filesList.contains(body)) {
            revert FR_WrongState(bytes32("FR_DocNotRegistered"));
        }
        _;
    }
    ...
    {
        if (body == address(0)) {
            revert FR_WrongInput(bytes32("FR_ZeroAddress"));
        }
    ...
    function circulateFile(
        ...
        if (repo.files[body].head.state != uint8(StateOfFile.Created)) {
            revert FR_WrongState(bytes32("FR_DocStateNotCreated"));
        }
        ...
        if (head.signingDays == 0) {
            revert FR_WrongInput(bytes32("FR_ZeroSigningDays"));
        }

        if (head.closingDays < signingDays + vr.frExecDays + vr.dtExecDays + vr.dtConfirmDays + 
                vr.invExitDays + vr.votePrepareDays + vr.votingDays) {
            revert FR_Overflow(bytes32("FR_ExceedingClosingDays"));
        }
    ...
    function proposeFile(
        ...
        if (repo.files[body].head.state != uint8(StateOfFile.Circulated)) {
            revert FR_WrongState(bytes32("FR_DocNotCirculated"));
        }

        uint48 timestamp = uint48(block.timestamp);

        if (timestamp < dtExecDeadline(repo, body)) {
            revert FR_WrongState(bytes32("FR_StillInDtExecPeriod"));
        }

        File storage file = repo.files[body];

        if (!(timestamp < terminateStartpoint(repo, body) || (file.head.frExecDays
             + file.head.dtExecDays + file.head.dtConfirmDays) == 0)) {
            revert FR_WrongState(bytes32("FR_MissedProposeDeadline"));
        }
    ...
    function voteCountingForFile(
        ...
        if (repo.files[body].head.state != uint8(StateOfFile.Proposed)) {
            revert FR_WrongState(bytes32("FR_DocNotProposed"));
        }
    ...
    function execFile(
        ...
        if (file.head.state != uint8(StateOfFile.Approved)) {
            revert FR_WrongState(bytes32("FR_DocNotApproved"));
        }

        uint48 timestamp = uint48(block.timestamp);

        if (timestamp >= closingDeadline(repo, body)) {
            revert FR_WrongState(bytes32("FR_MissedClosingDeadline"));
        }
    ...
    function terminateFile(
        ...
        if (file.head.state == uint8(StateOfFile.Closed)) {
            revert FR_WrongState(bytes32("FR_DocClosed"));
        }
