...
library UsersRepo {
    ...
    error USRR_WrongInput(bytes32 reason);
    error USRR_WrongState(bytes32 reason);
    error USRR_Overflow(bytes32 reason);
    error USRR_WrongParty(bytes32 reason);



    /// @notice Restrict to platform owner (users[0].primeKey.pubKey).
    modifier onlyOwner(Repo storage repo) {
        if (msg.sender != repo.users[0].primeKey.pubKey)
            revert USRR_WrongParty(bytes32("UR_NotOwner"));
        _;
    }

    /// @notice Restrict to platform keeper (users[0].backupKey.pubKey).
    modifier onlyKeeper(Repo storage repo) {
        if (msg.sender != repo.users[0].backupKey.pubKey)
            revert USRR_WrongParty(bytes32("UR_NotKeeper"));
        _;
    }
    ...
    function transferOwnership(Repo storage repo, address newOwner) public onlyOwner(repo) {
        if (newOwner == address(0)) 
            revert USRR_WrongInput(bytes32("UR_TO: zero address"));
    ...
    function handoverCenterKey(Repo storage repo, address newKeeper) public onlyKeeper(repo) {
        if (newKeeper == address(0)) 
            revert USRR_WrongInput(bytes32("UR_HCK: zero address"));
    ...
    function regUser(
        ...
        if (msgSender == address(0)) 
            revert USRR_WrongInput(bytes32("USRR_ZeroAddress"));
        if (usedKey(repo, msgSender))
            revert USRR_WrongInput(bytes32("USRR_UsedKey"));
    ...
    function setBackupKey(
        ...
        if (msgSender == address(0)) 
            revert USRR_WrongInput(bytes32("URSBK_ZeroCallerAddress"));
        if (bKey == address(0))
            revert USRR_WrongInput(bytes32("URSBK_ZeroBKeyAddress"));
        if (usedKey(repo, bKey))
            revert USRR_WrongInput(bytes32("URSBK_UsedKey"));

        uint caller = getUserNo(repo, msgSender);

        User storage user = repo.users[caller];

        if (user.backupKey.pubKey != address(0))
            revert USRR_WrongInput(bytes32("URSBK_AlreadySetBackupKey"));
    ...
    function upgradeBackupToPrime(
        ...
        if (!usedKey(repo, msgSender))
            revert USRR_WrongInput(bytes32("URUBP_CallerNotRegistered"));
        
        User storage user = repo.users[getUserNo(repo, msgSender)];

        if (user.backupKey.pubKey == address(0))
            revert USRR_WrongInput(bytes32("URUBP_ZeroBackupKey"));
    ...
    {
        if (author == 0) 
            revert USRR_WrongInput(bytes32("URGR_ZeroAuthor"));
