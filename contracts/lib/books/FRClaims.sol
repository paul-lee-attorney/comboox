...
library FRClaims {
    ...
    error FRClaims_WrongInput(bytes32 reason);

    error FRClaims_WrongParty(bytes32 reason);

    error FRClaims_WrongState(bytes32 reason);
    ...
    function claimFirstRefusal(
        ...
        if (cl.seqOfDeal == 0) {
            revert FRClaims_WrongInput(bytes32("FRC_ZeroSeqOfDeal"));
        }

        if (isClaimerOfDeal(cls, cl.seqOfDeal, cl.claimer)) {
            revert FRClaims_WrongParty(bytes32("FRC_DoubleClaim"));
        }
    ...
    function computeFirstRefusal(
        ...
        if (!isDeal(cls, seqOfDeal)) {
            revert FRClaims_WrongState(bytes32("FRC_DealNotExist"));
        }
        ...
        } else revert FRClaims_WrongInput(bytes32("FRClaims_AlreadyCreated"));
