...
library SharesRepo {
    ...
    error SHR_WrongInput(bytes32 reason);

    error SHR_WrongState(bytes32 reason);

    error SHR_Overflow(bytes32 reason);

    error SHR_WrongParty(bytes32 reason);
    ...
    modifier shareExist(
        ...
        if (!isShare(repo, seqOfShare)) {
            revert SHR_WrongState(bytes32("SHR_ShareNotExist"));
        }
        _;
    }
    ...
    {
        if (share.head.class == 0) {
            revert SHR_WrongInput(bytes32("SHR_ZeroClass"));
        }
        if (share.body.par == 0) {
            revert SHR_WrongInput(bytes32("SHR_ZeroPar"));
        }
        if (share.body.par < share.body.paid) {
            revert SHR_WrongInput(bytes32("SHR_PaidExceedPar"));
        }
        if (share.head.issueDate > block.timestamp) {
            revert SHR_WrongInput(bytes32("SHR_FutureIssueDate"));
        }
        if (share.head.issueDate > share.body.payInDeadline) {
            revert SHR_WrongInput(bytes32("SHR_IssueDateLaterPayInDeadline"));
        }
        if (share.head.shareholder == 0) {
            revert SHR_WrongInput(bytes32("SHR_ZeroShareholder"));
        }
        if (share.head.votingWeight == 0) {
            revert SHR_WrongInput(bytes32("SHR_ZeroVotingWeight"));
        }
    ...
    function payInCapital(
        ...
        if (deltaPaid == 0) {
            revert SHR_WrongInput(bytes32("SHR_ZeroAmt"));
        }

        if (block.timestamp > share.body.payInDeadline) {
            revert SHR_WrongState(bytes32("SHR_MissedDeadline"));
        }

        if (share.body.paid + deltaPaid > share.body.par) {
            revert SHR_Overflow(bytes32("SHR_PaidExceedPar"));
        }
    ...
    function increaseCleanPaid(
        ...
        if (deltaClean == 0) {
            revert SHR_WrongInput(bytes32("SHR_ZeroDeltaCleanPaid"));
        }
        ...
        if (isIncrease && share.body.cleanPaid + deltaClean <= share.body.paid) 
            ...
            share.body.cleanPaid -= deltaClean;
        else revert SHR_Overflow(bytes32("SHR_CleanPaidOverflow"));
    }
    ...
    function updatePayInDeadline(
        ...
        if (block.timestamp >= newLine) {
            revert SHR_WrongInput(bytes32("SHR_UpdatePayInDeadlineNotFuture"));
        }
