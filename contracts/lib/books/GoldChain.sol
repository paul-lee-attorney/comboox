...
library GoldChain {
   ...
   /* data[0] {
    ...
    error GC_WrongInput(bytes32 reason);

    error GC_WrongState(bytes32 reason);
    ...
    function createNode(
        ...
        if (uint64(paid) == 0) {
            revert GC_WrongInput(bytes32("GC_ZeroPaid"));
        }

        if (!isOffer) {
            if (data.margin == 0) {
                revert GC_WrongInput(bytes32("GC_ZeroMargin"));
            }
        }
