...
async function main() {
	...
	const addrUSDC = "0xaf88d065e77c8cC2239327C5EDb3A432268e5831" // USDC on Arbitrum One

	// await rc.connect(signers[1]).setTemplate( getTypeByName("MockUSDC"), addrUSDC, acct_0);
	await rc.connect(signers[1]).setTemplate( getTypeByName("USDC"), addrUSDC, acct_0);

	// await rc.connect(signers[1]).proxyDoc(getTypeByName("USDC"), 1);

	// const usdc = await readTool("MockUSDC", (await rc.getDoc(getTypeByName("MockUSDC"), 1, 1))[1]);
	// const usdc = await readTool("MockUSDC", (await rc.getTemp(getTypeByName("USDC"), 1))[1]);
	// await usdc.connect(signers[1]).setNewOwner(signers[0].address);
	
	console.log("registered USDC:", addrUSDC, "\n");
	...
	await cnc.connect(signers[1]).updateBank(addrUSDC); 
