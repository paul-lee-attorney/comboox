...
const { 
	RegCenter, EnumerableSet, GoldChain, ArrayUtils,
	LockersRepo, DealsRepo, RolesRepo, SigsRepo, 
	InvestorsRepo, Address, TopChain, UsdOrdersRepo, 
	OptionsRepo, MotionsRepo, SwapsRepo, DocsRepo,
	PledgesRepo, Checkpoints, MembersRepo, 
} = require("../250623-Arbi/contracts-address-consolidated.json");
...
async function main() {
	...
	let usdc = "0xaf88d065e77c8cC2239327C5EDb3A432268e5831"; // Arbitrum
	...
	// let usdLooKeeper = await readContract("UsdLOOKeeper", "0xad57c3dE4AE1c4F11F790083a27E4eBa6be42c70");
	...
	await rc.connect(signers[1]).setTemplate( 35, roiKeeper.address, 8);
	console.log("set template for ROIKeeper at address: ", roi.address, "\n");

	await rc.connect(signers[1]).setTemplate( 36, roi.address, 8);
	console.log("set template for RegisterOfInvestors at address: ", roi.address, "\n");

	await rc.connect(signers[1]).setTemplate( 37, accountant.address, 8);
	console.log("set template for Accountant at address: ", accountant.address, "\n");

	await rc.connect(signers[1]).setTemplate( 38, fundRORKeeper.address, 8);
	console.log("set template for FundRORKeeper at address: ", fundRORKeeper.address, "\n");

	await rc.connect(signers[1]).setTemplate( 39, ror.address, 8);
	console.log("set template for RegisterOfRedemptions at address: ", ror.address, "\n");

	await rc.connect(signers[1]).setTemplate( 40, fk.address, 8);
	console.log("set template for FundKeeper at address: ", fk.address, "\n");

	await rc.connect(signers[1]).setTemplate( 41, fundAccountant.address, 8);
	console.log("set template for FundAccountant at address: ", fundAccountant.address, "\n");

	await rc.connect(signers[1]).setTemplate( 42, fundGMMKeeper.address, 8);
	console.log("set template for FundGMMKeeper at address: ", fundGMMKeeper.address, "\n");

	await rc.connect(signers[1]).setTemplate( 43, fundLOOKeeper.address, 8);
	console.log("set template for FundLOOKeeper at address: ", fundLOOKeeper.address, "\n");

	await rc.connect(signers[1]).setTemplate( 44, fundROCKeeper.address, 8);
	console.log("set template for FundROCKeeper at address: ", fundROCKeeper.address, "\n");

	await rc.connect(signers[1]).setTemplate( 45, fundROIKeeper.address, 8);
	console.log("set template for FundROIKeeper at address: ", fundROIKeeper.address, "\n");

	await rc.connect(signers[1]).setTemplate( 46, cnc.address, 8);
	console.log("set template for CreateNewComp at address: ", cnc.address, "\n");

	await rc.connect(signers[1]).setTemplate( 46, cnf.address, 8);
	console.log("set template for CreateNewFund at address: ", cnf.address, "\n");
