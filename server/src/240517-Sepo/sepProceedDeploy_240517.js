...
const { addrs } = require("./addrs_240416_sep");
const { sites } = require("./sites_240517_sep");
...
async function main() {
	...
	// params = ['0x4Ef54d78314ac451923646e53334FA4e1E452428'];
	...
	// await rc.connect(signers[0]).regUser();
	...
	await rc.connect(signers[1]).setTemplate( 3, sites.BMMKeeper, 1);
	console.log("set template for BMMKeeper at address: ", sites.BMMKeeper, "\n");
	...
	await rc.connect(signers[1]).setTemplate( 5, sites.GMMKeeper, 1);
	console.log("set template for GMMKeeper at address: ", sites.GMMKeeper, "\n");

	await rc.connect(signers[1]).setTemplate( 6, sites.ROAKeeper, 1);
	console.log("set template for ROAKeeper at address: ", sites.ROAKeeper, "\n");

	await rc.connect(signers[1]).setTemplate( 7, sites.ROOKeeper, 1);
	console.log("set template for ROOKeeper at address: ", sites.ROOKeeper, "\n");

	await rc.connect(signers[1]).setTemplate( 8, sites.ROPKeeper, 1);
	console.log("set template for ROPKeeper at address: ", sites.ROPKeeper, "\n");

	await rc.connect(signers[1]).setTemplate( 9, sites.SHAKeeper , 1);
	console.log("set template for SHAKeeper at address: ", sites.SHAKeeper , "\n");

	await rc.connect(signers[1]).setTemplate( 10, sites.LOOKeeper, 1);
	console.log("set template for LOOKeeper at address: ", sites.LOOKeeper, "\n");
	...
	await rc.connect(signers[1]).setTemplate( 18, sites.RegisterOfShares, 1);
	console.log("set template for ROS at address: ", sites.RegisterOfShares, "\n");
	...
	await rc.connect(signers[1]).setTemplate( 24, sites.LockUp, 1);
	console.log("set template for LU at address: ", sites.LockUp, "\n");
